...
class WebRTCDataChannel:
        ...
        async def on_message(message):
            ...
            try:
                ...
                if isinstance(message, str):
                    # String messages (command responses, heartbeat, etc.)
                    # are processed immediately on the event loop for fast future resolution.
                    parsed_data = json.loads(message)
                    self.pub_sub.run_resolve(parsed_data)
                    await self.handle_response(parsed_data)

                elif isinstance(message, bytes):
                    # Binary messages (lidar, video data) are fully offloaded to
                    # the thread pool. No await here — the event loop is freed immediately
                    # so command responses can be processed without any delay.
                    def process_binary(msg_bytes):
                        try:
                            parsed = self.deal_array_buffer(msg_bytes)
                            # Pre-decoding Throttle
                            if parsed is not None:
                                topic_or_type = parsed.get("topic") or parsed.get("type")
                                if hasattr(self, "pub_sub") and not self.pub_sub.should_process(topic_or_type):
                                    return None
                                self.pub_sub.run_resolve(parsed)
                        except Exception:
                            logging.error("Error processing binary WebRTC data", exc_info=True)

                    self.pub_sub.callback_pool.submit(process_binary, message)
    ...
    def deal_array_buffer_for_normal(self, buffer):
        ...
        # Pre-decoding Throttle
        topic_or_type = decoded_json.get("topic") or decoded_json.get("type")
        if hasattr(self, "pub_sub") and not self.pub_sub.should_process(topic_or_type):
            return None
